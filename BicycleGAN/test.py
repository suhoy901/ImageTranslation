...
def main(opts):
    dataloader, data_len = data_loader(root=opts.root, batch_size='all', shuffle=False,
    ...
    if opts.epoch is not None:
        weight_name = '{epoch}-G.pkl'.format(epoch=opts.epoch)
    else:
        weight_name = 'G.pkl'

    weight_path = os.path.join(opts.weight_dir, weight_name)
    G = model.Generator(z_dim=8).type(dtype)
    G.load_state_dict(torch.load(weight_path))
    G.eval()

    if os.path.exists(opts.result_dir) is False:
        os.makedirs(opts.result_dir)

    # For example, img_name = random_55.png
    if opts.epoch is None:
        opts.epoch = 'latest'
    img_name = '{type}_{epoch}.png'.format(type=opts.sample_type, epoch=opts.epoch)
    img_path = os.path.join(opts.result_dir, img_name)

    # Make latent code and images
    z = make_z(n=data_len, img_num=opts.img_num, z_dim=8, sample_type=opts.sample_type)

    result_img = make_img(dataloader, G, z, img_size=128)
    torchvision.utils.save_image(result_img, img_path, nrow=opts.img_num + 1, padding=4)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--sample_type', type=str, choices=['random', 'interpolation'], default='random', help='Type of sampling : \'random\' or \'interpolation\'')
    parser.add_argument('--root', type=str, default='data/aaa', help='Data location')
    parser.add_argument('--result_dir', type=str, default='test', help='Ouput images location')
    parser.add_argument('--weight_dir', type=str, default='weight', help='Trained weight location of generator. pkl file location')
    parser.add_argument('--img_num', type=int, default=5, help='Generated images number per one input image')
    parser.add_argument('--epoch', type=int, help='Epoch that you want to see the result. If it is None, the most recent epoch')
